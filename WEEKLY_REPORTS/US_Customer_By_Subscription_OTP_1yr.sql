WITH Weeks AS (
  SELECT
    TO_CHAR(DATEADD(DAY, 7 * (ROW_NUMBER() OVER (ORDER BY SEQ4()) - 1), '2023-06-18'), 'YYYY-MM-DD') AS Week_Start_Date,
    TO_CHAR(DATEADD(DAY, 6, DATEADD(DAY, 7 * (ROW_NUMBER() OVER (ORDER BY SEQ4()) - 1), '2023-06-18')), 'YYYY-MM-DD') AS Week_End_Date
  FROM
    TABLE(GENERATOR(ROWCOUNT => 52)) -- generate 'x' weeks
),
    Valid_Customers AS (
